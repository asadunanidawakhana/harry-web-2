        ...
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8 animate-fadeInUp">
            <header className="text-center mb-12">
                <h1 className="text-5xl font-extrabold text-white flex items-center justify-center gap-4">
                    <BookText size={48} className="text-accent-glow" />
            ...
            {!loading && !error && (
                <div className="relative border-l-2 border-border pl-8 space-y-12">
                    ...
                    {entries.map((entry, index) => (
                        <div key={entry.id} className="relative animate-slideInUp" style={{ animationDelay: `${index * 100}ms` }}>
                            <div className="absolute -left-[37px] top-1 h-4 w-4 bg-accent-glow rounded-full border-4 border-surface"></div>
                            ...
                            <h2 className="text-2xl font-bold text-white mb-2">{entry.title} <span className="text-sm font-mono bg-surface px-2 py-0.5 rounded-md text-accent-glow">{entry.version}</span></h2>
